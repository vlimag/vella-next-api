...
Final API review remediation: store CTA attribution now classifies the active pathname at click time after same-locale soft navigation; www redirects cover Next asset URLs as well as pages without touching localhost; sitemap assertions require the apex host and no `/en` prefix. Editorial validation now requires the exact Play Store URL, app id, outer query shape, and nested referrer fields, and rejects schedule-dependent week-4 wording. Draft and named-human approval gates remain unchanged.
