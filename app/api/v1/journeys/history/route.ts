...
function historyEntry(value: unknown) {
  ...
  if (!id || !isStableSlug(slug) || typeof version !== 'number' || !Number.isInteger(version) || version < 1 || version > 1000
    ...
    || !title || !summary || !startedAt || !completedAt || sessionCount === null) return null;
...
export async function GET(request: Request) {
  try {
    ...
    if (params.getAll('limit').length > 1) return noStore(fail('Invalid request input', 400, { code: 'invalid_history_limit' }));
    const parsed = historyQuerySchema.safeParse({ limit: params.get('limit') ?? undefined });
    if (!parsed.success) return noStore(fail('Invalid request input', 400, parsed.error.flatten()));

    const { data, error } = await createServiceClient()
      .from('user_journeys')
      .select('id, status, start_date, completed_at, total_completed_days, journey_templates!inner(slug, version, language_code, title, description, theme_tags)')
